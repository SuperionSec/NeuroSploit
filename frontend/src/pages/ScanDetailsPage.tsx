    ...
    return (
      <div className="flex flex-col items-center justify-center h-64">
        ...
        <div className="flex gap-2">
          ...
          <Button customVariant="secondary" onClick={() => navigate('/')}>Go to Dashboard</Button>
        </div>
      </div>
    )
  }
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3" style={{ animation: 'fadeSlideIn 0.3s ease-out' }}>
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-3 mt-2 flex-wrap">
            <SeverityBadge severity={
              currentScan.status === 'running' ? 'info' :
              currentScan.status === 'completed' ? 'success' :
              currentScan.status === 'failed' ? 'critical' :
              currentScan.status === 'paused' ? 'warning' :
              'info'
            } />
        ...
        <div className="flex gap-2 flex-wrap flex-shrink-0">
          {agentData?.agent_id && (
            <Button customVariant="secondary" onClick={() => navigate(`/agent/${agentData.agent_id}`)}>
          ...
          {currentScan.status === 'running' && (
            <>
              ...
              <Button customVariant="danger" onClick={handleStopScan}>
          ...
          {autoGeneratedReport && (
            <>
              <Button customVariant="secondary" onClick={() => window.open(reportsApi.getViewUrl(autoGeneratedReport.id), '_blank')}>
                <FileText className="w-4 h-4 mr-2" />View Report
              </Button>
              <Button customVariant="secondary" onClick={() => window.open(reportsApi.getDownloadZipUrl(autoGeneratedReport.id), '_blank')}>
          ...
          {(currentScan.status === 'completed' || currentScan.status === 'stopped') && (
            <>
              ...
              <div className="relative">
                <Button onClick={() => setShowReportModelPicker(!showReportModelPicker)} isLoading={isGeneratingAiReport} customVariant="secondary"
      ...
      {autoGeneratedReport && (
        <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-4 flex items-center justify-between flex-wrap gap-3" style={{ animation: 'fadeSlideIn 0.3s ease-out' }}>
          ...
          <div className="flex items-center gap-2">
            <Button size="small" onClick={() => window.open(reportsApi.getViewUrl(autoGeneratedReport.id), '_blank')}>
              <ExternalLink className="w-4 h-4 mr-2" />View Report
            </Button>
            <Button size="small" customVariant="ghost" onClick={() => setAutoGeneratedReport(null)}>Dismiss</Button>
          </div>
        </div>
      )}
                        ...
                        <div className="p-4 pt-0 space-y-4 border-t border-dark-700">
                          ...
                          {(vuln.poc_request || vuln.poc_payload) && (
                            <div>
                              <div className="flex items-center justify-between mb-1">
                                ...
                                <Button customVariant="ghost" size="small" onClick={() => copyToClipboard(vuln.poc_request || vuln.poc_payload || '')}>
                          ...
                          {vuln.validation_status !== 'validated' && vuln.validation_status !== 'false_positive' && (
                            <div className="flex items-center gap-2 pt-2 border-t border-dark-700 flex-wrap">
                              ...
                              <Button customVariant="ghost" size="small" className="text-green-400 hover:bg-green-500/10 border border-green-500/30"
                                onClick={async (e) => {
                                  e.stopPropagation()
                                  try {
                              ...
                              <Button customVariant="ghost" size="small" className="text-dark-400 hover:bg-red-500/10 border border-dark-600"
                                onClick={(e) => {
                                  e.stopPropagation()
                              ...
                              <Button customVariant="ghost" size="small" className="text-blue-400 hover:bg-blue-500/10 border border-blue-500/30"
                                onClick={(e) => {
                                  e.stopPropagation()
                          ...
                          {(vuln.validation_status === 'validated' || vuln.validation_status === 'false_positive') && (
                            <div className="flex items-center gap-2 pt-2 border-t border-dark-700">
                              ...
                              <Button customVariant="ghost" size="small" className="text-dark-500 hover:text-dark-300 text-xs"
                                onClick={async (e) => {
                                  e.stopPropagation()
                                  try {
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setFeedbackVulnId(null)}>
          <div className="bg-dark-800 border border-dark-700 rounded-xl p-6 w-full max-w-md mx-4" onClick={e => e.stopPropagation()} style={{ animation: 'fadeSlideIn 0.3s ease-out' }}>
            ...
            <div className="flex gap-3 justify-end">
              <Button customVariant="secondary" onClick={() => setFeedbackVulnId(null)}>Cancel</Button>
              <Button
                customVariant={feedbackIsTp ? 'primary' : 'danger'}
