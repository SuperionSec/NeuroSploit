...
Testers for XSS, SQL Injection, Command Injection, SSTI, NoSQL, LDAP, XPath.
"""
import re
...
class XSSReflectedTester(BaseTester):
    ...
    def analyze_response(
        ...
        if payload in response_body:
            ctx = self._analyze_xss_context(response_body, payload)
            if ctx["executable"]:
    ...
    def _analyze_xss_context(self, body: str, payload: str) -> Dict:
        payload_escaped = re.escape(payload)
        auto_exec_patterns = [
            rf"<script[^>]*>.*?{payload_escaped}.*?</script>",
            rf"javascript\s*:\s*{payload_escaped}",
            rf"on\w+\s*=\s*['\"]?\s*{payload_escaped}",
            rf"eval\s*\(\s*['\"]?{payload_escaped}",
        ]
        for pattern in auto_exec_patterns:
            if re.search(pattern, body, re.IGNORECASE | re.DOTALL):
                return {"executable": True, "interactive": False, "context": "auto-exec", "detail": "auto-executing context"}

        interactive_patterns = [
            rf"<a[^>]*href\s*=\s*['\"]javascript\s*:.*?{payload_escaped}",
            rf"<button[^>]*on\w+\s*=\s*['\"]?{payload_escaped}",
            rf"<form[^>]*action\s*=\s*['\"]javascript\s*:",
            rf"<svg[^>]*on\w+\s*=\s*['\"]?{payload_escaped}",
            rf"<img[^>]*on\w+\s*=\s*['\"]?{payload_escaped}",
        ]
        for pattern in interactive_patterns:
            if re.search(pattern, body, re.IGNORECASE | re.DOTALL):
                return {"executable": False, "interactive": True, "context": "interactive", "detail": "user interaction required"}

        if "&lt;" in body or "&gt;" in body:
            return {"executable": False, "interactive": False, "context": "html-encoded", "detail": "payload HTML encoded"}
        if re.search(r'["\'].*?' + payload_escaped, body):
            return {"executable": False, "interactive": False, "context": "in-attribute", "detail": "inside quoted attribute"}
        if re.search(r'/\*.*?' + payload_escaped, body):
            return {"executable": False, "interactive": False, "context": "in-comment", "detail": "inside HTML comment"}

        return {"executable": False, "interactive": False, "context": "raw-text", "detail": "reflected as raw text"}
...
class XSSDomTester(BaseTester):
    ...
    def analyze_response(
        ...
        for sink in self.dom_sinks:
            pattern = rf'{re.escape(sink)}[^;]*(?:location|document\.URL|document\.referrer|window\.name)'
