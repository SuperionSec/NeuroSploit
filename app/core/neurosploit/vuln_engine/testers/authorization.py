...
Testers for IDOR, BOLA, Privilege Escalation, BFLA, Mass Assignment, Forced Browsing.
"""
import re
                ...
                if re.search(pattern, response_body, re.IGNORECASE):
                    if "original_id" in context:
                        if context["original_id"] not in payload:
                            return True, 0.8, "IDOR: Accessed different user's data"
    ...
    def analyze_response(
        ...
        if response_status == 200:
            ...
            for pattern in elevated_access:
                if re.search(pattern, response_body, re.IGNORECASE):
                    return True, 0.9, "Privilege escalation: Elevated role in response"
            ...
            for pattern in admin_data_patterns:
                if re.search(pattern, response_body, re.IGNORECASE):
                    ...
                    if any(ind in payload.lower() for ind in admin_url_indicators):
                        return True, 0.85, "BFLA: Admin data returned for non-admin request"
...
class MassAssignmentTester(BaseTester):
    ...
    def analyze_response(
        ...
        if response_status in [200, 201]:
            privileged_fields = [
                (r'"(?:is)?admin"\s*:\s*true', "admin flag"),
