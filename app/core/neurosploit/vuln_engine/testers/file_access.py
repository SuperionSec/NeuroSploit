...
Testers for LFI, RFI, Path Traversal, XXE, File Upload, Arbitrary File Read/Delete, Zip Slip.
"""
import re
import base64
    ...
    def analyze_response(
        ...
        for pattern in file_contents:
            if re.search(pattern, response_body):
                return True, 0.9, "Path traversal successful: File content detected"
...
class XXETester(BaseTester):
    ...
    def analyze_response(
        ...
        for pattern in xxe_indicators:
            if re.search(pattern, response_body):
                return True, 0.9, "XXE confirmed: External entity processed"
...
class ArbitraryFileReadTester(BaseTester):
    ...
    def analyze_response(
        ...
        for b64_match in base64_pattern[:5]:
            try:
